...
为 ShopAgent 提供移动端导购入口，交互形态类似豆包 APP：用户以自然语言发起多轮对话，客户端通过 SSE 流式接口消费后端响应，将大模型推荐文本逐段渲染（打字机效果），并把先到的结构化商品事件渲染为商品卡片。商品卡片展示标题、品牌、价格和图片；点击卡片弹出商品详情 BottomSheet，查看商品描述与全部 SKU。
...
- 商品卡片展示：标题、品牌、双价格（`display_price` / `base_price`）、图片（含占位图）、SKU 数量提示。
- 商品详情 BottomSheet：点击商品卡片弹出，含商品图片、标题、品牌、描述（后端暂未下发，客户端预留占位）、双价格与完整 SKU 列表。
...
- 点击商品卡片弹出 ModalBottomSheet 商品详情：商品大图、标题、品牌、描述（未下发时显示占位文案）、双价格、完整 SKU 列表（规格摘要 + 单价）；下滑或点击空白处关闭。
...
- 历史会话列表展示本地保存的会话（标题取首条用户消息前 20 字 + 更新时间，按最新消息时间倒序，当前会话高亮）；仅当会话有新一轮消息落库时才刷新其排序位置，单纯打开查看不改变顺序。点击后从本地载入完整消息（含商品卡片），恢复 `conversation_id` 与最后一条用户文本，可直接继续对话或重试失败消息。
- 冷启动自动恢复最近会话（历史列表首条，按更新时间倒序），消息与 `conversation_id` 一并还原，可直接继续对话；无历史会话时进入空白新会话。
...
- 助手回复文本为 Markdown（无序列表、`**` 加粗），气泡内用 compose-richtext 渲染；流式期间未闭合的标记（如半截 `**`）会暂时显示为字面量，闭合后自动生效。
...
- `product` 事件含 `description` 商品摘要字段（取自数据集 `rag_knowledge.marketing_description`），在详情弹窗展示。
...
- 技术选型：Kotlin + Jetpack Compose（Material 3）+ Coroutines/Flow + OkHttp（okhttp-sse）+ kotlinx.serialization + Coil；Markdown 渲染用 compose-richtext `1.0.0-alpha02`（该版本按 Kotlin 2.0.21 构建，与项目精确对齐，更新的 alpha03+ 按更高 Kotlin 构建暂不升级）；MVVM，手动依赖注入（单 Repository + ViewModelFactory，不引入 Hilt）；minSdk 26。不引入 Retrofit（对 SSE 无帮助）、Room（Phase 3 再评估）。
...
- SKU 列表不在商品卡片内联展开（会撑高卡片、顶开聊天列表），改为点击卡片弹出 ModalBottomSheet 商品详情；`description` 由后端 `product` 事件下发，弹窗直接展示，缺失时显示占位文案。
...
代码位于 `app/src/main/java/com/shopagent/`，结构与 [../plan.md](../plan.md) 一致：`data/`（ChatApi、dto、ChatRepository）、`data/local/`（AppDatabase、Entities、ConversationDao、ConversationStore 接口 + RoomConversationStore）、`domain/`（ChatMessage）、`ui/`（ChatViewModel、ChatScreen、`components/` 下的 MessageBubble/ProductCard/ProductDetailSheet/FloatingInputBar/TypingIndicator）；另有 `ui/cart/CartScreen.kt`、`ui/profile/ProfileScreen.kt` 两个占位页。导航由 `MainActivity` 的左侧抽屉（ModalNavigationDrawer + CenterAlignedTopAppBar，抽屉内容在 `ui/AppDrawer.kt`）经状态切换实现，未引入 navigation-compose。
...
- 手动验收：后端 `docker compose up -d` + uvicorn 起服务，模拟器跑通各 Phase 验收项（流式文本、卡片先于文本、点击卡片弹出详情 BottomSheet、占位图、错误重试）。
...
| 2026-08-06 | 补回 debug 构建的 `networkSecurityConfig`（`app/src/debug/` 叠加 manifest，放行明文 HTTP）；`.env` 的 `PUBLIC_BASE_URL` 改注释原值并新增 `http://10.0.2.2:8000` | 图片加载失败根因：模拟器无法访问后端下发的 `127.0.0.1` 图片 URL；明文放行配置此前丢失，API 28+ 模拟器会全量断连 |
| 2026-08-06 | `ChatViewModel` 增加未保存变更标记：仅一轮对话结束才写库，打开/离开会话的兜底保存不再刷新 `updatedAt` | 修复仅查看历史会话就跳到列表顶部的排序问题 |
| 2026-08-09 | SKU 堆叠组件（SkuStack）移除，改为点击商品卡片弹出 ModalBottomSheet 商品详情（ProductDetailSheet，含图片/基本信息/描述/双价格/完整 SKU 列表）；`ProductDto`/`ProductCard` 新增 `description` 字段 | 内联展开会撑高卡片、顶开聊天列表布局；详情弹窗承载更多商品信息 |
| 2026-08-09 | 后端 `product` 事件开始下发 `description`（取自数据集 `rag_knowledge.marketing_description`），详情弹窗展示真实商品摘要 | 详情弹窗的描述占位接入真实数据 |
| 2026-08-09 | 冷启动自动恢复最近会话：`ChatViewModel` 初始化时读取历史列表首条并载入，无历史时保持空白新会话 | 重启 App 直接进入最近聊天，而不是空白页 |
| 2026-08-09 | 商品图片加载优化：`SubcomposeAsyncImage` 改为「占位图衬底 + AsyncImage 覆盖」，卡片列表组合时预热图片进 Coil 缓存；后端图片接口加 `Cache-Control: max-age=86400` | 修复切换历史会话时图片闪烁：子组合机制命中缓存也会闪一帧占位；磁盘缓存跨重启免回源验证 |
| 2026-08-09 | 助手回复接入 Markdown 渲染：引入 compose-richtext `1.0.0-alpha02`（`richtext-ui-material3` + `richtext-commonmark`），`AssistantBubble` 的 `Text` 换成 `RichText { Markdown(...) }`；后端推荐/场景提示词同步加列表+加粗格式契约 | 后端开始返回带 `**`/`-` 的 Markdown 文本，纯 `Text` 会原样显示标记；版本锁定 alpha02 因其按项目同款 Kotlin 2.0.21 构建 |
