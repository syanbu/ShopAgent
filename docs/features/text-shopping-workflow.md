...
| 2026-07-23 | 添加召回聚合、证据校验与候选决策（开发中） | 将 Qdrant 结果重新绑定 catalog 事实，限制证据白名单，并按结构化条件、语义证据和本次重排结果确定最多三个候选 |
| 2026-07-23 | 添加 LangGraph 条件路由与 custom stream（开发中） | 固化非购物、零召回、证据为空和正常推荐路径，保证商品事实事件先于白名单约束下的生成文本 |
| 2026-07-23 | 添加 FastAPI SSE、商品图片与健康接口（开发中） | 将工作流事件稳定映射为 HTTP 流，隔离本地图片路径，并明确依赖未就绪与部分失败语义 |
